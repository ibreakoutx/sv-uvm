...
function wb_cycle::new(wb_cfg cfg);
   super.new(this.log);
   this.cfg = cfg;
endfunction: new
...
function vmm_data wb_cycle::copy(vmm_data to);
   wb_cycle tr;
...
function bit wb_cycle::is_valid(bit silent,
                                int kind);
...
function vmm_data wb_cycle_resp::copy(vmm_data to);
   wb_cycle_resp tr;
...
function bit wb_cycle_resp::is_valid(bit silent,
                                     int kind);
`ifdef NO_RANDOMIZE_NULL
   is_valid = 1;
`else
