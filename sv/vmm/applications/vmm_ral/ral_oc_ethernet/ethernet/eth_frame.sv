...
`endif
   ...
   extern virtual function int unsigned byte_pack(ref logic [7:0]    array[],
                                                  input int unsigned offset = 0,
                                                  input int          kind   = -1);
   extern virtual function int unsigned byte_unpack(const ref logic [7:0] array[],
                                                    input int unsigned    offset = 0,
                                                    input int             len    = -1,
                                                    input int             kind   = -1);
...
function vmm_data eth_frame::copy(vmm_data to);
   eth_frame cpy;
...
function bit eth_frame::is_valid(bit silent,
                                 int kind);
...
function int unsigned eth_frame::byte_size(int kind);
   byte_size = 14 + data.size();
   if (format == TAGGED) byte_size += 4;
endfunction: byte_size


function int unsigned eth_frame::byte_pack(ref logic [7:0]    array[],
                                           input int unsigned offset,
                                           input int          kind);
   int i;

   if (array.size() < offset + this.byte_size(kind)) begin
      array = new [offset + this.byte_size(kind)] (array);
   end

   i = offset;
   {array[i  ], array[i+1], array[i+2], array[i+3], array[i+ 4], array[i+ 5]} = this.dst;
   {array[i+6], array[i+7], array[i+8], array[i+9], array[i+10], array[i+11]} = this.src;
   i += 12;

   case (this.format)
   UNTAGGED: begin
      {array[i], array[i+1]} = this.len_typ;
      i += 2;
   end

   TAGGED: begin
      {array[i], array[i+1]} = 16'h8100;
      i += 2;
`ifdef ETH_USE_COMPOSITION
      {array[i], array[i+1]} = {this.vlan.user_priority, this.vlan.cfi, this.vlan.id};
`else
      {array[i], array[i+1]} = {this.user_priority, this.cfi, this.vlan_id};
`endif
      i += 2;
      {array[i], array[i+1]} = this.len_typ;
      i += 2;
   ...
   // Example 4-38
   CONTROL: begin
      {array[i], array[i+1]} = 16'h8808;
      i += 2;
      {array[i], array[i+1]} = this.opcode;
      i += 2;
...
function int unsigned eth_frame::byte_unpack(const ref logic [7:0] array[],
                                             input int unsigned    offset,
                                             input int             len,
                                             input int             kind);
   int array_len;
...
`endif
   ...
   if (len > 0) begin
      array_len = len + offset;
      if (array_len > array.size()) array_len = array.size();
   end
   ...
   for (k = 0; i < array_len && k < 6; k++) begin
      this.dst[47-k*8 -:8] = array[i];
      i++;
      byte_unpack++;
   end
   if (i >= array_len) return byte_unpack;
   for (k = 0; i < array_len && k < 6; k++) begin
      this.src[47-k*8 -:8] = array[i];
      i++;
      byte_unpack++;
   end
   if (i >= array_len) return byte_unpack;
   ...
   if (i+1 >= array_len) return byte_unpack;
   // Example 4-41
...
`endif
      ...
      if (i+1 >= array_len) return byte_unpack;
`ifdef ETH_USE_COMPOSITION
      {this.vlan.user_priority, this.vlan.cfi, this.vlan.id} = {array[i], array[i+1]};
`else
      {this.user_priority, this.cfi, this.vlan_id} = {array[i], array[i+1]};
`endif
      i += 2;
      byte_unpack += 2;
      if (i+1 >= array_len) return byte_unpack;
      this.len_typ = {array[i], array[i+1]};
      i += 2;
      byte_unpack += 2;
   end else if ({array[i], array[i+1]} == 16'h8808) begin
      this.len_typ = 16'h8808;
      this.format = eth_frame::CONTROL;
      i += 2;
      byte_unpack += 2;
      if (i+1 >= array_len) return byte_unpack;
      this.opcode = {array[i], array[i+1]};
      i += 2;
      byte_unpack += 2;
      case (this.opcode)
         PAUSE: begin
            if (i+1 >= array_len) return byte_unpack;
            this.pause_time = {array[i], array[i+1]};
            i += 2;
            byte_unpack += 2;
            no_data = 1;
         end
   ...
   if (no_data == 0) begin
      int l = (this.len_typ >= 'h0600 ||
               this.len_typ > array_len - i) ? array_len - i : this.len_typ;
      this.data = new [l];
