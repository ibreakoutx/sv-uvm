...
class mii_phy_layer extends vmm_xactor;
   ...
   function new(string                   instance,
      ...
      super.new("MII PHY Layer", instance, stream_id);

      this.cfg = cfg;
      this.sigs = sigs;

      if (tx_chan == null) tx_chan = new({this.log.get_name(), " PHY->MAC frames"}, instance);
      this.tx_chan = tx_chan;
      if (rx_chan == null) rx_chan = new({this.log.get_name(), " MAC->PHY frames"}, instance);
...
function void mii_phy_layer::reconfigure(mii_cfg   cfg,
                                         eth_frame rx_factory);
   if (cfg != null) this.cfg = cfg;
   if (rx_factory != null) this.rx_factory = rx_factory;
endfunction: reconfigure

   
function void mii_phy_layer::reset_xactor(reset_e typ);
