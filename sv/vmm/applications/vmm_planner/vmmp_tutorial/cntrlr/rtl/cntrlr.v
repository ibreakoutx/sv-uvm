...
// SV assertions starts here

  wire b_reading;
  assign b_reading = ( (adxStrb && busRdWr_N && !reset) );
  wire b_writing;
  assign b_writing = ( (adxStrb && !busRdWr_N && !reset) );

  property e1;
      @ ( posedge clk) $countones({ce0_N,ce1_N,ce2_N,ce3_N}) >= 3;
  endproperty
  property e2;
     @ ( posedge clk) (!reset && $countones({ce0_N,ce1_N,ce2_N,ce3_N})==3) |-> ##1 $countones({ce0_N,ce1_N,ce2_N,ce3_N})==4;
  endproperty
  property e3;
     @ ( posedge clk) (b_reading) |-> ##3 busOe;
  endproperty
  property e4;
     @ ( posedge clk) (b_writing) |-> ##3 !rdWr_N;
  endproperty
  protocol_ceX_N: assert property( e1 );
  protocol_ceX_N2: assert property( e2 );
  protocol_reading: assert property( e3 );
  protocol_writing: assert property( e4 );



  wire b_ce_idle;
  assign b_ce_idle = ( (ce0_N && ce1_N && ce2_N && ce3_N) );

  reg [7:0] mem [0:255];
  initial mem = '{256{8'h00}};

  always @ ( posedge clk)
     mem[busAddr] <= b_writing ? busData : mem[busAddr];
  ...
  property e6;
    @(posedge clk) (b_reading) |-> ##3 (busData == mem[$past( busAddr,3 )]);
  endproperty

  protocol_adxstrb: assert property( e5 );
  protocol_checkData: assert property( e6 );

// SV Assertions end here
