...
class mii_monitor extends vmm_xactor;
   ...
   function new(string                   instance,
      ...
      super.new("MII Monitor", instance, stream_id);

      this.cfg = cfg;
      this.sigs = sigs;

      if (to_phy_chan == null) to_phy_chan = new({this.log.get_name(), " MAC->PHY frames"}, instance);
      this.to_phy_chan = to_phy_chan;
      if (to_mac_chan == null) to_mac_chan = new({this.log.get_name(), " PHY->MAC frames"}, instance);
...
function void mii_monitor::reconfigure(mii_cfg   cfg,
                                       eth_frame to_phy_factory,
                                       eth_frame to_mac_factory);
...
function void mii_monitor::reset_xactor(reset_e typ);
     ...
     // Wait for the SFD
      ...
      if (!err) begin
         ...
         if (log.start_msg(vmm_log::DEBUG_TYP , vmm_log::TRACE_SEV )) begin
            log.text("Observed Mac->Phy frame...");
   ...
   while (1) begin
     ...
     // Wait for the SFD
      ...
      if (!err) begin
         ...
         if (log.start_msg(vmm_log::DEBUG_TYP , vmm_log::TRACE_SEV )) begin
            log.text("Observed Phy->Mac frame...");
            log.text(fr.psdisplay("   "));
            log.end_msg();
         end
