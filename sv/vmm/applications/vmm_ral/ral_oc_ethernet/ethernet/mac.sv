...
class eth_mac extends vmm_xactor;
   ...
   function new(string              instance,
      ...
      super.new("Ethernet MAC Layer", instance, stream_id);

      this.cfg = cfg;

      if (tx_chan == null) tx_chan = new({this.log.get_name(), " Tx frames"}, instance);
      this.tx_chan = tx_chan;
      if (rx_chan == null) rx_chan = new({this.log.get_name(), " Rx frames"}, instance);
      this.rx_chan = rx_chan;

      if (pls_tx_chan == null) pls_tx_chan = new({this.log.get_name(), " PHY->MAC frames"}, instance);
      this.pls_tx_chan = pls_tx_chan;
      if (pls_rx_chan == null) pls_rx_chan = new({this.log.get_name(), " MAC->PHY frames"}, instance);
...
class eth_mac_monitor extends vmm_xactor;
   ...
   function new(string              instance,
                ...
                eth_pls_indications indications     = null);
      super.new("Ethernet MAC Layer Monitor", instance, stream_id);

      this.cfg = cfg;

      if (to_phy_chan == null) to_phy_chan = new({this.log.get_name(), " ->MAC frames"}, instance);
      this.to_phy_chan = to_phy_chan;
      if (to_mac_chan == null) to_mac_chan = new({this.log.get_name(), " <-MAC frames"}, instance);
      ...
      if (pls_to_mac_chan == null) pls_to_mac_chan = new({this.log.get_name(), " MAC->PHY frames"}, instance);
...
function void eth_mac::reset_xactor(reset_e typ);
   ...
   if (typ >= FIRM_RST) begin
      this.indications.reset(, vmm_notify::HARD );
   end

   if (typ >= HARD_RST) begin
   ...
   while (1) begin
      ...
      while (status.n_attempts < this.cfg.attempt_limit && !status.successful) begin
         ...
         if (log.start_msg(vmm_log::DEBUG_TYP , vmm_log::TRACE_SEV )) begin
            string prefix;
            log.text("Attempting to transmitting frame...");
            $sformat(prefix, "   Attempt #%0d: ", status.n_attempts);
            log.text(fr.psdisplay(prefix));
            log.end_msg();
         end
      ...
      if (log.start_msg(vmm_log::DEBUG_TYP , vmm_log::TRACE_SEV )) begin
         string prefix;
         log.text("Done transmitting frame...");
         $sformat(prefix, "   %0s: ", (status.successful) ? "Success" : "Failure");
         log.text(fr.psdisplay(prefix));
         log.end_msg();
      end
      
      this.tx_chan.complete();
...
function void eth_mac_monitor::reset_xactor(reset_e typ);
   super.reset_xactor(typ);
